...
class TwitterCrawler:
    def __init__(self, _from, _to):
        self.days_range = []
        start = datetime.datetime.strptime(_from, "%Y-%m-%d")
        end = datetime.datetime.strptime(_to, "%Y-%m-%d")
        self.date_generated = [start + datetime.timedelta(days=x) for x in range(0, (end - start).days)]

    def make_condition(self, key_word):
        ...
        # 트윗 수집 기준 정의
        self.tweetCriteria = got.manager.TweetCriteria().setQuerySearch(key_word) \
            .setSince(start_date) \
            .setUntil(end_date) \
            .setMaxTweets(-1)

    def collect_data(self, key_word):
        ...
        for index in tqdm(tweet):
            ...
            # csv 파일 만들기
            twitter_df.to_csv(key_word + "sample_twitter_data_{}_to_{}.csv".format(self.days_range[0], self.days_range[-1]), index=False,
                              encoding='utf-8')
